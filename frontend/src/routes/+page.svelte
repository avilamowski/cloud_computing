<script>
  ...
  try {
    const publicationsResponse = await api.get(`get_publications?page=${p}&search=${searchTerm}&tags=${tagsFromURL}`);
...
function toggleTag(tag) {
  ...
  // Filtrar los tags disponibles según el término de búsqueda
  $: filteredTags = availableTags.filter(tag => 
    tag.toLowerCase().includes(tagFilter.toLowerCase()) && !selectedTags.includes(tag)
  );
  ...
  <div class="home-page">
    ...
    <div class="container page">
      <div class="row">
        ...
        <div class="col-md-3">
          <Searchbar {searchTerm} {searchSelectedTags} href={(t) => `/?search=${t}`} />
          {#if isAuthenticated}
            ...
            <div class="available-tags">
              ...
              <div class="tag-container">
                {#each availableTags as tag}
                  <span
                    class="tag-pill {searchSelectedTags.includes(tag) ? 'selected' : ''}"
  ...
  <div class="modal-content">
    ...
    <form method="POST" on:submit={createPublication}>
      ...
      <div class="tag-search-container">
        ...
        <div>
          <input type="text" id="tagFilter" bind:value={tagFilter} placeholder="Search tags..." class="search-input" />
        </div>
          ...
          <div class="tags-section">
            ...
            <div class="tag-container">
              {#each filteredTags as tag}
                <span class="tag-pill" on:click={() => toggleTag(tag)}>{tag}</span>
              {/each}
            </div>
          </div>
        {/if}
          ...
          <div class="tags-section">
            ...
            <div class="selected-tags">
              {#each selectedTags as tag}
                <span class="tag-pill selected" on:click={() => toggleTag(tag)}>{tag}</span>
              {/each}
            </div>
          </div>
        {/if}
