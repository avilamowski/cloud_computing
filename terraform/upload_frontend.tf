...
resource "terraform_data" "upload_build" {
  ...
  provisioner "local-exec" {
    command = "${path.cwd}/scripts/deploy_frontend.sh ${local.frontend_bucket_name} ${local.api_endpoint}/${local.stage} ${aws_cognito_user_pool_client.userpool_client.id} ${terraform_data.cognito_base_url.output} ${local.redirect_lambda.function_url}"
  }
